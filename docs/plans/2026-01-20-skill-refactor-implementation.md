...
description: 清除 Doc-Smith workspace 中的授权令牌和部署配置。当用户要求清除配置、重置授权、清除部署信息时使用此技能。
---

# Doc-Smith 清除配置

清除 Doc-Smith workspace 中的授权令牌和部署配置。
...
首先检查当前目录是否为有效的 Doc-Smith workspace：

```bash
ls -la config.yaml
```

如果 `config.yaml` 不存在，提示用户当前不在 Doc-Smith workspace 中。
...
**可清除的内容：**
- **授权令牌 (authTokens)**：清除 `config.yaml` 中的 `authToken` 字段
- **部署配置 (deploymentConfig)**：清除 `config.yaml` 中的 `appUrl`、`deployedAt` 等部署相关字段

询问用户："请选择要清除的内容：1) 授权令牌 2) 部署配置 3) 全部清除"

### 3. 执行清除

根据用户选择，读取 `config.yaml`，删除对应字段，然后写回文件。
...
cat skills/doc-smith-clear/SKILL.md | head -10
```
