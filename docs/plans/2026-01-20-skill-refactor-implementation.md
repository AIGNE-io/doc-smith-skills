...
### Task 1.2: 更新 doc-smith-content 添加独立调用检查
...
在 `skills/doc-smith-content/SKILL.md` 的"工作流程"开头添加：
...
### Task 1.3: 更新 doc-smith 主 Skill 添加 Skill 引用
...
在 `skills/doc-smith/SKILL.md` 末尾添加：
...
将流程中提到 `generateDocumentDetails` 工具的地方改为引用 `doc-smith-content` 技能：
...
git commit -m "refactor: update doc-smith to reference doc-smith-content skill"
```
...
Expected: 包含 `doc-smith/` 和 `doc-smith-content/`

**Step 2: 功能测试**

在实际项目中测试：
1. 调用 `/doc-smith` 生成文档
2. 调用 `/doc-smith-content` 单独重新生成某篇文档
...
### Task 2.1: 创建 doc-smith-images Skill
...
**输入：**
- prompt: 图片描述/生成提示词（必需）
- size: 图片尺寸（可选，默认 "2K"）
- ratio: 宽高比（可选，默认 "16:9"）
- outputPath: 图片保存路径（可选）
...
**Step 4: 简化 AIGNE 项目为原子能力**

修改 `skills/doc-smith-images/scripts/aigne-generate/`：
- 移除文档扫描相关逻辑（scan-image-slots.mjs 等）
- 保留核心生图功能（generate-image.yaml）
- 更新输入参数为 prompt/size/ratio
- 创建简化的入口

**Step 5: 更新 doc-smith 主 Skill 引用生图**

在 `skills/doc-smith/SKILL.md` 中更新图片生成相关描述：

```markdown
### 8.4 检查是否存在 AFS Image Slot 需要生成图片

检查本次生成的文档中是否包含 `AFS Image Slot`：

1. 使用 Grep 搜索文档中的 `<!-- afs:image` 标记
2. 提取每个 slot 的 `desc` 属性作为 prompt
3. 对每个 slot 调用 `doc-smith-images` 技能生成图片
4. 将生成的图片路径更新到文档中
```
...
```
用户：帮我生成一张架构图，展示微服务之间的调用关系
```

**Step 2: 测试文档流程中的生图**

生成包含 AFS Image Slot 的文档，验证图片能否自动生成。

---

## Phase 3: 检查功能

### Task 3.1: 创建 doc-smith-check Skill
...
修改复制的脚本，更新 `import` 路径或内联必要的常量。

**Step 5: 更新 doc-smith 主 Skill 引用检查**

在 `skills/doc-smith/SKILL.md` 中更新校验相关描述：

```markdown
### 4.2 执行程序化校验

生成 YAML 后，调用 `doc-smith-check` 技能进行结构校验。
```
...
### Task 4.1: 创建 doc-smith-publish Skill

**Files:**
- Create: `skills/doc-smith-publish/SKILL.md`
- Create: `skills/doc-smith-publish/scripts/` (如需要)

**Step 1: 创建目录**

```bash
mkdir -p skills/doc-smith-publish/scripts
```
...
```bash
cp agents/publish/*.mjs skills/doc-smith-publish/scripts/
```
...
### Task 5.1: 创建 doc-smith-translate Skill
...
```bash
mkdir -p skills/doc-smith-translate
```
...
### Task 6.1: 创建 doc-smith-clear Skill

**Files:**
- Create: `skills/doc-smith-clear/SKILL.md`
- Create: `skills/doc-smith-clear/scripts/clear-auth.mjs`
...
git commit -m "feat: add doc-smith-clear skill"
```
...
测试完整流程：
1. `/doc-smith` - 生成文档
2. `/doc-smith-images` - 生成图片
3. `/doc-smith-check` - 检查文档
4. `/doc-smith-translate` - 翻译文档
5. `/doc-smith-publish` - 发布文档
6. `/doc-smith-clear` - 清除配置
...
|-------|------|-----------|
| 1 | 主入口 + 详情生成 | 能生成文档 |
| 2 | doc-smith-images | 能生成图片 |
| 3 | doc-smith-check | 能检查文档 |
| 4 | doc-smith-publish | 能发布文档 |
| 5 | doc-smith-translate | 能翻译文档 |
| 6 | doc-smith-clear | 能清除配置 |
| 7 | 清理验证 | 完整功能 |
