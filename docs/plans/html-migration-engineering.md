...
在 DocSmith 现有流程基础上，集成 HTML 构建步骤，发布到 MyVibe，替换当前 Discuss Kit 文档站点。
...
### 3. 托管：MyVibe 作为唯一托管平台

**结论**：文档作为 MyVibe 的一种内容类型，使用 `/myvibe-publish` 发布。
...
| 方面 | 方案 | 操作方式 |
|------|------|---------|
| 导航入口 | 现有站点导航指向 MyVibe 文档地址 | Blocklet 后台配置，不改代码 |
