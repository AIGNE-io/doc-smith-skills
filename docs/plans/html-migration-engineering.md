...
在 DocSmith 现有流程基础上，集成 HTML 构建步骤，发布到 MyVibe 托管，替换当前 Discuss Kit 文档站点。
...
| doc-smith-create 增加构建步骤 | `skills/doc-smith-create/SKILL.md` | 集成构建步骤 + 生成发布配置，提示用 `/myvibe-publish` 发布 |
...
| 事项 | 原因 |
|------|------|
| 改造 doc-smith-publish | `/myvibe-publish` 已支持 `--hub` 参数指定目标站点 |
| 改造 MyVibe | `/myvibe-publish` 已支持静态 HTML 发布，不需要额外改造 |
...
### 3. 发布：统一使用 `/myvibe-publish`

**结论**：只用 `/myvibe-publish` 一个命令，通过项目级配置区分发布目标。

**理由**：
- `/myvibe-publish` 已支持 `--hub` 参数指定目标站点
- DocSmith 站点和 MyVibe 站点本质是同一能力（静态托管），不应有两个命令
- 发布目标是项目属性，不是每次操作的选择

**项目级发布配置**：
- doc-smith-create 生成时在 workspace 中写入发布配置（如 `.aigne/doc-smith/publish.yaml`）
- `/myvibe-publish` 读到配置后自动选择目标站点，用户无需手动指定 `--hub`
...
- [ ] 改造 Phase 9（结束提示）：报告构建结果，提示使用 `/myvibe-publish` 发布
- [ ] 生成发布配置：在 workspace 中写入 `publish.yaml`（含目标站点地址）
- [ ] 更新相关技能表：新增 doc-smith-build，保留 /myvibe-publish
- [ ] 更新任务规划模板：增加 Phase 8
...
AI 先生成 Markdown（中间产物，token 低、输出稳定）→ build.mjs 构建 HTML + 清理 MD（核心逻辑复用）→ `/myvibe-publish` 发布到 MyVibe 托管（项目配置自动指定目标站点）→ Blocklet 后台配置导航入口 → 托管平台提供版本回退。需要改造的组件：doc-smith-content（MD→HTML 转换流程）、build.mjs（适配 + 清理 MD）、doc-smith-create（集成构建步骤 + 生成发布配置）、doc-smith-images（去掉 AIGNE CLI，直接调 AIGNE Hub）。
