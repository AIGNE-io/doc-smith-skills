...
> 在现有 Markdown 生成流程上集成 HTML 构建，发布到 MyVibe 托管，替换 Discuss Kit。
...
└── assets/
    ...
    └── theme.css          # AI 生成（可改）
        ↓
/myvibe-publish → 发布到 MyVibe 托管（项目配置自动指定目标站点）→ 线上可访问
        ↓
...
| 托管在哪？ | MyVibe 托管 | `/myvibe-publish` 已支持 `--hub` 指定站点 |
| 怎么发布？ | `/myvibe-publish` + 项目配置 | 配置写在 `publish.yaml`，用户无感 |
...
| `skills/doc-smith-create/SKILL.md` | **改造** | 集成构建步骤 + 生成发布配置（publish.yaml） |
...
- doc-smith-create 改造：集成构建步骤 + 生成发布配置（publish.yaml）
