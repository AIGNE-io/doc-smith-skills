...
const TOKEN_KEY = "DOCSMITH_ACCESS_TOKEN";
...
export async function getCachedAccessToken(hubUrl) {
  ...
  let accessToken = process.env.DOCSMITH_ACCESS_TOKEN;
...
export async function getAccessToken(hubUrl, locale = "en") {
  ...
  // .well-known/service is at domain root, not under blocklet mount path
  const connectUrl = joinURL(hubUrl, WELLKNOWN_SERVICE_PATH);

  console.log(chalk.cyan("\nAuthorization required for DocSmith..."));

  try {
    const result = await createConnect({
      connectUrl,
      connectAction: "gen-simple-access-key",
      source: "DocSmith Publish Skill",
      closeOnSuccess: true,
      appName: "DocSmith",
      appLogo:
        "https://docsmith.aigne.io/image-bin/uploads/6c26b2b5226ed0d554ac0b4ea0b6db38.jpeg",
