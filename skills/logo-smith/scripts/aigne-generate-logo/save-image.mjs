...
/**
 * 将 Image Agent 生成的 Logo 保存到指定路径
 */
export default async function saveImage(input) {
  ...
  try {
    ...
    return { message: `Logo 已保存到 ${savePath}（${sizeKB} KB）` };
  } catch (err) {
    return { message: `保存失败：${err.message}` };
  }
}

saveImage.description = "Save generated logo to specified path";
saveImage.input_schema = {
  ...
  properties: {
    ...
    savePath: {
      type: "string",
      description: "Output file path to save the logo",
    },
  },
